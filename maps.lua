...
function maps.load(path)
	print("INFO: MAPS -> Loading " .. path)

	if maps.list[path] then
		print("INFO: MAPS -> " .. path .. " already loaded.")
	else
		maps.list[path] = maps.new(path)
		print("INFO: MAPS -> " .. path .. " successfully loaded in " .. maps.list[path].debug.load_time .. " seconds.")
		print("INFO: MAPS -> " .. maps.list[path].debug.tilecount .. " tiles, " .. #maps.list[path].bufferbatches .. " meshes.")
	end
